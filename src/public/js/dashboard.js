...
$(document).ready(function(){
    ...
    {   
        ...
        /* Real-time clock */
        var realTimeClock = $('main .device-connection div:last-child h3')
        setInterval(function(){
            var now = new Date()
    ...
    /* ALarms */
        ...
        var valueOfFilter = 'All'
        liFetchDA.on('click',function(e){
            if(!($(this).attr('class')==='page-item disabled')){
                var pageNowDA = parseInt(e.target.dataset.page)     
                var dataAPI = 'http://localhost:8000/user/alarms/digital-data?pageDA='+pageNowDA+'&filter='+valueOfFilter
                fetch(dataAPI)
                    .then((response)=> {
        ...
        digitalAlarmFilterTag.on('change', function(){
            valueOfFilter=this.value
            digitalFilterAPI = 'http://localhost:8000/user/alarms/digital-data?filter='+valueOfFilter
...
function getClockTime(time){
    time = new Date(time)
    return `CLOCK: ${time.getHours()}:${time.getMinutes()}:${time.getSeconds()} ${time.getDate()}/${time.getMonth()+1}/${time.getFullYear()+100}`
}
/* Check Role */
