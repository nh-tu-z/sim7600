...
/* Check Digital Alarm */
module.exports.checkDigitalAlarm = (tagname, previousValue, recentValue, timeStamp) => {
  DASetting.find({tagname: tagname}, (err, alarm) => {
    if(alarm.length){
...
/* Check Analog Alarm */
module.exports.checkAnalogAlarm = (tagname, dir, preState, recentValue, timeStamp) => {
  AASetting.findOne({tagname: tagname}, (err, alarm) => {
    ...
    if(state === preState){
      ...
      console.log(typeof recentValue)
      newAlarm.save()
        .then(() =>{
          console.log('Alarm: A Analog Alert was created!!!')
        })
        .catch(err => {console.log(err)})
    }
  })
